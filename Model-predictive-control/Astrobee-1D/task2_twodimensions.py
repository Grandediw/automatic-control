...
from simulation import EmbeddedSimEnvironment_2axis
...
# Two-axis Matrices
A, B = abee.two_axis_ground_dynamics()
...
K = ctl.get_closed_loop_gain_2axis([0.982,0.976,0.983,0.977])


# Set the desired reference based on the dock position and zero velocity on docked position
dock_target = np.array([[0.0, 0.0, 0.0, 0.0]]).T
...
sim_env = EmbeddedSimEnvironment_2axis(model=abee,
                                 dynamics=abee.linearized_discrete_dynamics_2axis,
                                 controller=ctl.control_law_2axis,
                                 time=40.0)
t, y, u = sim_env.run(x0)
sim_env.visualize()

# Disturbance effect
abee.set_disturbance_2axis()
sim_env = EmbeddedSimEnvironment_2axis(model=abee,
                                 dynamics=abee.linearized_discrete_dynamics_2axis,
                                 controller=ctl.control_law_2axis,
                                 time=40.0)
