...
class Astrobee(object):
    ...
    def forward_propagate(self, x_s, npoints, radius=0.5):
        ...
        p = np.array(x_s[0:3]).reshape(3,1)
        v = np.array(x_s[3:6]).reshape(3,1)
        q = np.array(x_s[6:10]).reshape(4,1)
        w = np.array(x_s[10:]).reshape(3,1)
        ...
        x_r[:,0] = np.concat((pb.flatten(),v.flatten(),q.flatten(),w.flatten()))
        ...
        for i in range(npoints-1):
            ...
            pb_next = p_next.reshape(3,1) + r_mat_q_np(q=q_next)[:,0].reshape(3,1) * radius

            rotational_effect = np.cross(w, (pb_next - p_next).flatten(), axis=0) # Here we calculate the rotational effect of the angular velocity   
            pb_next = pb_next + rotational_effect * self.dt # and add it to the estimated projected point 
            # Remove these previous two to remove the added p_B refined model

            x_r[:,i+1] = np.concat((pb_next.flatten(),v.flatten(),q_next.flatten(),w.flatten()))
